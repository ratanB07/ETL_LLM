...
@app.route('/ai_data_prep')
def ai_data_prep():
    return render_template('ai_data_prep.html')

@app.route('/llm_code_generation')
def llm_code_generation():
    return render_template('llm_code_generation.html')

@app.route('/model_training')
def model_training():
    return render_template('model_training.html')

@app.route('/prompt_interface')
def prompt_interface():
    return render_template('prompt_interface.html')

@app.route('/time_series_forecast')
def time_series_forecast():
    return render_template('time_series_forecast.html')

@app.route('/nlp_tools')
def nlp_tools():
    return render_template('nlp_tools.html')

@app.route('/mlops_integration')
def mlops_integration():
    return render_template('mlops_integration.html')

# In main.py (add this with your other routes)
@app.route('/code_free_modeling')
def code_free_modeling_page():
    return render_template('code_free_modeling.html')

@app.route('/visual_ml_pipeline')
def visual_ml_pipeline():
     return render_template('visual_ml_pipeline.html')
...
@app.route('/auto_feature_selection')
def auto_feature_selection():
     return render_template('auto_feature_selection.html')
...
def deploy_to_alteryx():
     return render_template('deploy_to_alteryx.html')

@app.route('/ml_data_matching')
def ml_data_matching():
     return render_template('ml_data_matching.html')


@app.route('/ai_data_catalog')
def ai_data_catalog():
     return render_template('ai_data_catalog.html')

@app.route('/api/code_free_modeling', methods=['POST'])
def code_free_modeling_api():
    # Forward the request to the app.py service
    response = requests.post(
        'http://localhost:5001/code_free_modeling',
        json=request.json,
...
def machine_learning():
    try:
        ...
        if not filename or not target_column:
            return jsonify({"status": "error", "message": "Filename and target column are required"}), 400

        # Load the data (implementation depends on your file handling)
        # Example for CSV:
        ...
        # Forward to ML service with just the first 1000 rows to prevent timeouts
        response = requests.post(
            'http://localhost:5001/analyze',
            json={
                'data': df.head(1000).to_dict(orient='records'),  # Limit to first 1000 rows
        ...
        return jsonify({"status": "error", "message": f"ML service error: {str(e)}"}), 500
    except Exception as e:
